    ...
    try:
        if request.user.groups.all()[0].name == 'Agent':
            ...
            for product in products:
                price.append(product.Price_in_creates_Agent_sell)
                prods.append(product.Product_Name)
                quantity.append(getattr(order, product.Product_Name))
                sub_total.append(product.Price_in_creates_Agent_sell *
    ...
    try:
        if request.user.groups.all()[0].name == 'Agent':
            ...
            if request.method == 'POST':
                ...
                ag = Agent_order.objects.create(Agent=request_agent,status='Pending', Store=storee)
                for product in all_product:
                    a = request.POST[product.Product_Name]
                    arr[product.Product_Name] = a
                    tp = product.Price_in_creates_Agent_buy * \
                ...
                obj = {
                    "process": "Cart",
                    "successUrl": "http://localhost/Agent/success/",
                    "ipnUrl": "http://192.168.8.129:8002/Agent/ipn",
                    "cancelUrl": "http://192.168.8.129:8002/Agent/cancel",
                    "merchantId": "SB1560",
    ...
    try:
        if request.user.groups.all()[0].name == 'Agent':
            ...
            if request.method == 'POST':
                ...
                Drive_license = request.FILES.get('licence')
...
def Product_Price(request):
    ...
    return render(request,'Agent/price.html',context)

def BGI_store(request):
    all_store =Company_Store.objects.all()
    context = {
        'all_store':all_store
    }
    return render(request,'Agent/BGI_Store.html',context)
    
def store_detile_agent(request, pk):
    try:
        if request.user.groups.all()[0].name == 'Agent':
            manager = 'TBA'
            all_store =Company_Store.objects.all()
            ...
            return render(request, 'Agent/BGI_Store_detail.html', context)
        messages.error(request, 'permission denied ')
        return redirect('logout')
    except IndexError as e:
        messages.error(request, 'Login Before ')
